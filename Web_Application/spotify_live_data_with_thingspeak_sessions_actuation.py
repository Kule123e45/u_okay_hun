...
# import private credentials variables from config.py file
...
# =============================================================================
# TextAlert - Takes message and recipient input and sends SMS message
# =============================================================================
...
try:
    trigger_valence = 0.4 # set based on analysis of the user's normal listening library
    trigger = 0 # initialise trigger value
    message = "U okay hun? It looks like you're listening to some sad music, maybe switch it up a bit?" # SMS message to be send to the user
    to = config.MY_MOBILE_NUMBER # user's contact number
    also_to = None
    session_bucket = []
        ...
        try:
            ...
            if trackplaying:
                ...
                if session_bucket_size < SET_BUCKET_SIZE:
                    ...
                    if session_bucket:
                        # Compute mean and standard deviation of each feature within the session
                        ...
                        # Check for previous sad sessions (data pulled from thingspeak)
                        Previous_Session = 'https://api.thingspeak.com/channels/1277562/fields/1/last.json'
                        request = requests.get(Previous_Session).json()
                        last_session_valence = float(request['field1'])
                        if last_session_valence < trigger_valence:
                            try:
